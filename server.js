...
const APP  = 'SmartEmail'; // ★ scope all license reads/writes to this app
...
// ---------- GENERATE (always available; no gating) ----------
...
${finalAgent ? '**Sender Info:**\n' + finalAgent : ''}`.trim();
    ...
    // Not gated, but report actual tier so UI can show Pro/Premium badges correctly
    const lic = await checkLicense(finalEmail, licenseKey);
...
app.post('/enhance', async (req, res) => {
  try {
    const { email, enhance_request, enhance_content, licenseKey } = req.body || {};
    if (!email || !enhance_request || !enhance_content) {
      return res.status(400).json({ error: 'Missing required fields.' });
    }
    // FIX: use email (finalEmail not in scope)
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert(
        ...
        { onConflict: 'email,app_name', ignoreDuplicates: true } // idempotent
      )
      .select()
      .maybeSingle();
...
// ---------- LICENSE VALIDATION (scoped first, then fallback) ----------
  ...
  try {
    ...
    if (licenseKeyRaw) {
      ...
      let { data } = await supabase
        ...
        .eq('license_key', licenseKeyRaw).eq('app_name', APP).maybeSingle();
      row = data;
      ...
      let { data } = await supabase
        .from('licenses').select('email, license_key, smartemail_tier, smartemail_expires, tier, status, expires_at')
        .eq('email', email).eq('app_name', APP)
